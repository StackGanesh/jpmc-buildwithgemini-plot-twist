    ...
    for p in parts:
        ...
        if isinstance(root, TextPart) and getattr(root, "text", None):
            ...
            if isinstance(data, dict):
                ...
                inner = data.get("data")
                if mime == _A2UI_MIME and inner:
                    out.append({"kind": "a2ui", "data": inner})
                elif inner and isinstance(inner, dict) and any(k in inner for k in ("beginRendering", "surfaceUpdate", "dataModelUpdate")):
                    out.append({"kind": "a2ui", "data": inner})
                elif any(k in data for k in ("beginRendering", "surfaceUpdate", "dataModelUpdate")):
...
async def chat(req: Request):
    ...
    async with httpx.AsyncClient(headers=_auth_headers(), timeout=120) as client:
        ...
        # Fallback: pull parts from history or artifacts on the final task.
        if not parts and last_task is not None:
            history = getattr(last_task, "history", None) or []
            if history:
                last_msg = history[-1]
                parts.extend(_extract_parts(getattr(last_msg, "parts", [])))
            if not parts:
